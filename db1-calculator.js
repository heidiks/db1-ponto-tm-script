if($(".tabExterna").length) {
    ...
    $(".tabExterna th:last-child, .tabExterna td:last-child").each(function(index) {
        ...
        if((horasTrabalhadas.isValid() && !horasTrabalhadas.isBefore(DIA_FICTICIO + JORNADA_NORMAL, 'time')) || (!isDiaUtil($(this)) && horasTrabalhadas.isAfter(DIA_FICTICIO + '00:00:01', 'time'))) 
            $(this).append("&nbsp;<span class=\"label label-warning\" style=\"font-size:9px\" title=\"Hora extra\" onClick=\"enviarDadosBanco("+ calculaDiferenca(horasTrabalhadas.format("HH:mm:ss"), '08:48:00') + ",\"" + $(this).parent().children().first().text().slice(0,5).concat("/").concat(new Date().getFullYear())+ "\" , '+') \">+"+ calculaSaldo(horasTrabalhadas) + "</span>");
            ...
            $(this).append("&nbsp;<span class=\"label label-danger\" style=\"font-size:9px\" title=\"Jornada abaixo\" onClick=\"enviarDadosBanco("+ calculaDiferenca(horasTrabalhadas.format("HH:mm:ss"), '08:48:00') + ",\"" + $(this).parent().children().first().text().slice(0,5).concat("/").concat(new Date().getFullYear())+ "\" , '-')\">-" + calculaSaldoNegativo(horasTrabalhadas) + "</span>");
